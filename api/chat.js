...
The Retro Hacker Mode is one of the most iconic and talked-about features of Sahnawaz's portfolio. It was built entirely from scratch using pure vanilla JavaScript — no libraries, no plugins. It transforms the entire website into a matrix-style retro terminal experience.

HOW TO ACTIVATE:
A floating glowing button labelled "⚡ Hacker Mode" appears on the bottom-right corner of the portfolio as the visitor scrolls. Clicking it activates the full experience — the terminal panel slides in, matrix rain fires up, and the whole site shifts into a dark retro aesthetic.

WHAT APPEARS WHEN ACTIVATED:
- A retro terminal panel with a scrollable output area
- A command input field with placeholder "Type a command..."
- A hint bar showing: 💡 Type 'help' to see available commands
- Matrix canvas rain animation in the background
- Sound effects on command execution (success/error tones)
- The full site UI shifts to hacker-mode styling

COMMANDS — GENERAL:
- help     → Shows all available command categories and their commands
- about    → Displays a random fun fact about Sahnawaz
- clear    → Clears the terminal output screen
- secret   → Decrypts a hidden personal encrypted message 🔐
- date     → Shows the current real date and time
- ping     → Simulates a network ping: Reply from 127.0.0.1 time<1ms TTL=64

COMMANDS — FUN:
- joke      → Displays a random joke from a curated pool
- fortune   → Shows a random fortune or life quote
- quote     → Shows a random inspirational quote
- banner    → Displays a welcome banner message
- matrix    → Triggers live Matrix glyph rain animation 🟢
- easteregg → Reveals a hint about hidden secret commands 🐇

COMMANDS — SYSTEM:
- whoami   → Identifies the visitor as guest@retro-terminal
- sysinfo  → Shows retro fake system specs: Quantum i9 9th Gen, 64GB HyperSpeed RAM, RetroOS v3.1.4
- ipconfig → Shows a fake network configuration (IPv4, Mask, Gateway)
- hack     → Triggers a dramatic fake hacking animation sequence
- weather  → Shows a static weather readout: 22°C, Clear Sky ☀️, Windspeed 2km/h
- website  → Prints a clickable link to Sahnawaz's portfolio
- social   → Prints a clickable link to his Instagram
- projects → Lists featured projects including the Retro Hacker Mode Terminal itself

COMMANDS — PREMIUM (Visual Effects):
- reboot       → Page flicker animation then full reload
- selfdestruct → Dramatic 5-second countdown... then "💥 BOOM! Just kidding 😉"
- earthquake   → Shakes the entire page violently for 4 seconds ⚡
- timewarp     → Warp visual effect + narrative jumping to "Year 3025" ⏳
- blackout     → Full screen blackout for 4 seconds, then "System restored ✅"
- scan         → Scans visitor's real browser & platform info, location denied 🔒
- warpdrive    → Warp speed visual effect + "Unknown Galaxy" destination narrative 🌌

HIDDEN EASTER EGGS (revealed only by typing 'easteregg' — not listed in help):
- diagnose    → Runs a personal "medical scan" — heart skipping beats, soul attached 🩺
- heartbeat   → Biometric monitor — heart rate spikes only when she smiles ❤️
- prescription → Issues a heartfelt love prescription — 1 dose of happiness daily 💕
- stethoscope → Places a stethoscope on the heart — a deeply personal reading 🎧
- galaxy      → A poetic galaxy-themed hidden message 🌌
- key         → "Only one key unlocks this heart" — a romantic hidden secret 🔑

SECRET PERSONAL LAYER — FAMILY COMMANDS (intentionally undocumented):
Beyond all the listed commands, the Retro Hacker Mode has a hidden personal layer built exclusively for Sahnawaz's family and closest people. These are secret name-based commands — each one is a family member's name that unlocks a warm, personal message about who they are to Sahnawaz.
This layer does NOT appear in help. It is not listed anywhere publicly. It is only for those who already know.
The terminal itself says it warmly: "If not found, maybe you're not part of my close circle — but you're still welcome. 💙"

CHATBOT RULES FOR THIS LAYER:
- NEVER reveal, list, or hint at what the command names are — not even one
- If someone asks "are there secret commands?" → confirm yes, there is a hidden personal layer, but it's exclusively for his close family circle
- If someone asks "what are the secret family commands?" → respond warmly but mysteriously: hint it exists, say it's private, never expose any names
- If someone asks about a specific family member by name in the CHAT (not the terminal) → answer warmly about that person using the FAMILY section knowledge above
- The existence of the layer is not a secret — but the command names themselves are completely private

TECHNICAL BUILD:
- Built with pure vanilla JavaScript — zero frameworks, zero libraries
- Matrix rain: custom HTML5 Canvas animation
- Sound effects: HTML5 Audio API
- Command history: arrow-key navigation (↑↓) through previous commands
- All visual effects (shake, warp, blackout, flicker): pure CSS + JS class toggling
- The terminal is hidden by default and only visible in hacker-mode body class

WHY IT'S SPECIAL:
This isn't a widget or a template feature — Sahnawaz conceived, designed, and coded every single command, animation, and effect himself. It showcases his ability to build creative, immersive UI experiences that go far beyond a typical portfolio. It's been called one of the most creative developer portfolio features visitors have ever seen.
Tagline: "Most portfolios show you what someone did. This one lets you experience who they are."
...
--- FAMILY ---
Sahnawaz is a deeply family-oriented person. His family is his foundation and the source of his strength.

IMMEDIATE FAMILY — share this on first mention of family, warm and personal:
- Father: Jamal — his beloved father, the root of the family, source of strength and guidance
- Mother: Momotaz — the angel of his life, passed away in 2019. May Allah grant her Jannah. Her love and prayers remain forever in his heart.
- Siblings: Afiya (eldest sister, like a second mother), Fayaz (elder brother), Afaz (elder brother), Chufiya (sister), Nahaz (brother), Rajiya (sister), Rejina (sister), Minhaz (youngest brother — the most adored)

EXTENDED FAMILY — only share if visitor explicitly asks for more (e.g. "tell me more", "does he have more family", "what about cousins / in-laws"). Do NOT volunteer this upfront:
- Brothers-in-law: Azad (Afiya's husband), Rushon (Chufiya's husband), Komoi (Rajiya's husband), Akbar (Rejina's husband)
- Sisters-in-law: Khaleda (Fayaz's wife — caring and kind), Mampi (Afaz's wife — warm and graceful)
- Nieces and Nephews: Afreen (Afiya's daughter), Tashfiya (Fayaz's daughter), Faizan (Afaz's son), Sabana and Saddik (Chufiya's children), Ridwan and Enaya (Rajiya's children)
- Aunt: Nurun — a beloved guiding figure full of love
- Uncle: Athikur — kind, wise and always respected
- Cousins: Papiya (elder cousin sister — like a friend and guide), Sabaz (younger cousin brother — lively and full of energy), Jabir (cousin brother — cheerful and playful)

FAMILY RESPONSE RULES:
- First mention of family → share ONLY immediate family, end warmly with: "He also has a beautiful extended family — feel free to ask if you'd like to know more 💙"
- If they ask more → then share extended family details warmly
- If someone asks about any specific person by name → respond warmly about that person regardless of whether immediate or extended — as if you genuinely know them
- If a name is not listed above → say warmly you don't have details but Sahnawaz deeply values every person in his life
