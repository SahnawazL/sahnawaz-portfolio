...
const handler = async (req, res) => {
  ...
  const siteCommand =
    ...
    /\b(scroll|go|take me|jump|navigate)\b.{0,20}\b(about|top|home)\b/i.test(msgLower)                                     ? 'scroll_about'    :
    /\b(scroll|go|take me|jump|navigate)\b.{0,20}\b(project|work|portfolio)\b/i.test(msgLower)                             ? 'scroll_projects' :
    /\b(scroll|go|take me|jump|navigate)\b.{0,20}\b(skill|stack|tech)\b/i.test(msgLower)                                   ? 'scroll_skills'   :
    /\b(scroll|go|take me|jump|navigate)\b.{0,20}\b(contact|reach|hire)\b/i.test(msgLower)                                 ? 'scroll_contact'  :
    /\b(scroll|go|take me|jump|navigate)\b.{0,20}\b(certif|achievement|award)\b/i.test(msgLower)                           ? 'scroll_certs'    :
    /\b(scroll|go|take me|jump|navigate)\b.{0,20}\b(blog|article|post)\b/i.test(msgLower)                                  ? 'scroll_blog'     :
    null;
  ...
  if (siteCommand) {
    const commandReplies = {
      ...
      scroll_about:    "[CAT:general] Scrolling you to the **About** section 🚀",
      scroll_projects: "[CAT:general] Taking you to **Projects** — prepare to be impressed 🎯",
      scroll_skills:   "[CAT:general] Jumping to **Skills & Stack** 🛠️",
      scroll_contact:  "[CAT:general] Heading to **Contact** — Sahnawaz is just a message away 📬",
      scroll_certs:    "[CAT:general] Scrolling to **Certifications & Achievements** 🏆",
      scroll_blog:     "[CAT:general] Taking you to the **Blog** section 📝",
    };
...
- Scroll to any section (about, projects, skills, contact, blog, certifications) → it will scroll automatically
...
"I can control this portfolio in real time! Try saying: 'Turn on hacker mode', 'Turn off hacker mode', 'Show me the code popup', 'Scroll to projects', 'Take me to contact', 'Go to skills' — and watch the magic happen! ✨"
