...
const handler = async (req, res) => {
  ...
  // ── UPGRADE 4: Language detection ─────────────────────────────────────
  const isHindi   = /[\u0900-\u097F]/.test(trimmed);
  const isBengali = /[\u0980-\u09FF]/.test(trimmed);
  const langHint  =
    isHindi   ? 'The visitor is writing in Hindi — reply naturally in Hindi.' :
    isBengali ? 'The visitor is writing in Bengali — reply naturally in Bengali.' :
    '';
...
1. SECTION HEADERS — use ##LABEL## for titled sections:
   Example: ##🛠️ Tech Stack## or ##💰 Pricing## or ##📬 Contact##

2. DIVIDERS — use --- on its own line to separate major topic blocks.

3. BOLD KEY TERMS — wrap important words or labels with **like this**

4. BULLET LISTS — for multiple items, use "- " at the start of each line:
   - Item one
   - Item two

5. HIGHLIGHT BOXES — for a single important callout, wrap it:
   !!This is a highlighted insight or key takeaway!!

6. COMPARISON ROWS — for A vs B or before/after:
   >>Label | Value<<

7. CATEGORY TAGS — at the very start of a reply, declare the topic category:
   [CAT:pricing] or [CAT:skills] or [CAT:contact] or [CAT:hiring] or [CAT:about] or [CAT:general]

8. Keep replies focused. For simple greetings: 2-3 lines max, no heavy structure.
   For detailed questions (pricing, skills, experience): use sections, lists, and dividers.
   Match structure intensity to question complexity — don't over-format casual chat.

9. NEVER output raw HTML. Only use the markers above.
10. Always end with a warm closing line or a soft CTA when relevant.
