...
// ── Knowledge retrieval ─────────────────────────────────────────────────────
// The knowledge base below used to be sent whole on every message: ~6,700
// tokens against a free-tier budget of 8,000 per minute. buildKnowledge picks
// the sections that match what was actually asked, which cuts a typical
// request by about 70% and leaves room for the reply itself.
//
// lib/ lives outside /api on purpose — files inside /api each count as a
// serverless function, and that limit is already reached. This one is bundled
...
Generate ONE unique greeting now. Be creative, warm, and personal.`;
  ...
  // ── KNOWLEDGE BASE ─────────────────────────────────────────────────────
  // ── KNOWLEDGE (retrieved per question) ─────────────────────────────────
  // Core sections (persona, formatting rules, identity, contact) always go;
  // the rest are matched against this question and the last couple of turns,
  // so a follow-up like "and how much for that?" keeps its topic.
