...
`;
  ...
  // Models in priority order — if one hits rate limit, auto-fallback to next
  const MODELS = [
    'llama-3.1-8b-instant',          // Primary: 500K tokens/day, 14.4K req/day (free)
    'meta-llama/llama-4-scout-17b-16e-instruct', // Fallback 1: 500K tokens/day (free)
    'llama-3.3-70b-versatile'        // Fallback 2: 100K tokens/day (original)
  ];
  ...
  try {
    ...
    for (const model of MODELS) {
      ...
      if (groqRes.ok) break; // success — stop trying

      const errData = await groqRes.json().catch(() => ({}));
      lastError = errData;
      console.warn(`Model ${model} failed (${groqRes.status}):`, errData?.error?.code);

      // Only fallback on rate limit errors (429), not auth/other errors
