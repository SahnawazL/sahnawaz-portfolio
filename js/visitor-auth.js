...
function initOneTap() {
  ...
  loadScript('https://accounts.google.com/gsi/client', function() {
    window.google.accounts.id.initialize({
      ...
      /* CRITICAL FIX for Chrome Android white/blank page bug:
         ux_mode 'popup' keeps the account picker as an overlay
         instead of navigating away — no redirect, no white page.
         The credential is returned via the callback directly.    */
      ux_mode:               'popup'
    });
    ...
    loadScript('https://www.gstatic.com/firebasejs/9.23.0/firebase-auth-compat.js', function() {
      ...
      if (googleBtn) {
        googleBtn.onclick = function() {
          ...
          /* Use popup on ALL platforms — ux_mode:'popup' on One Tap means
             no redirect blank page on Android. signInWithPopup is safe here
             as a fallback since the GSI iframe conflict no longer applies. */
      ...
      /* ── Handle redirect result (kept for safety on older sessions) ── */
